...
export default class FilmAboutPopup extends AbstractView {
  ...
  addCloseButtonClickHandler (callbackFunction) {
    this._callback.click = callbackFunction;
    this.getElement().querySelector('#film-details-close-btn').addEventListener('click', this._clickHandler);
  }


  removeCloseButtonClickHandler (callbackFunction) {
    this._callback.click = callbackFunction;
    this.getElement().querySelector('#film-details-close-btn').removeEventListener('click', this._clickHandler);
  }
}
